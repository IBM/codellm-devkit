...
class JCodeanalyzer:
    ...
    def __init__(
        ...
        if analysis_level == "symbol_table":
            ...
            self.call_graph: DiGraph = self._generate_call_graph(
                using_symbol_table=False
            )
    ...
    def _download_or_update_code_analyzer(filepath: Path) -> str:
        ...
        if response.status_code == 200:
            for asset in response.json().get("assets", []):
                if asset["name"] == "codeanalyzer.jar":
                    ...
                    if match:
                        ...
                        raise Exception(
                            f"Release URL {download_url} does not contain a datetime pattern."
                        )

                    # Look for codeanalyzer.YYYYMMDDTHHMMSS.jar in the filepath
                    current_codeanalyzer_jars = [
                        jarfile for jarfile in filepath.glob("*.jar")
                    ]
                    if not any(current_codeanalyzer_jars):
                        logger.info(
                            f"Codeanalzyer jar is not found. Downloading the latest version."
                        )
                    ...
                    if match:
                        ...
                        if datetime.strptime(
                        ...
                        ) > datetime.strptime(current_datetime_str, date_format):
                            logger.info(
                                f"Codeanalzyer jar is outdated. Downloading the latest version."
                            )
                            ...
                            logger.info(
                                f"Codeanalzyer jar is already at the latest version."
                            )
                    else:
                        filename = current_codeanalyzer_jar_name
            ...
            raise Exception(
                f"Failed to fetch release warn: {response.status_code} {response.text}"
            )
    ...
    def _get_codeanalyzer_exec(self) -> List[str]:
        ...
        if self.use_graalvm_binary:
            with resources.as_file(
                resources.files("cldk.analysis.java.codeanalyzer.bin") / "codeanalyzer"
            ) as codeanalyzer_bin_path:
            ...
            if self.analysis_backend_path:
                ...
                codeanalyzer_exec = shlex.split(
                    f"java -jar {analysis_backend_path / 'codeanalyzer.jar'}"
                )
            else:
                # Since the path to codeanalyzer.jar was not provided, we'll download the latest version from GitHub.
                with resources.as_file(
                    resources.files("cldk.analysis.java.codeanalyzer.jar")
                ) as codeanalyzer_jar_path:
                    # Download the codeanalyzer jar if it doesn't exist, update if it's outdated,
                    # do nothing if it's up-to-date.
                    codeanalyzer_jar_file = self._download_or_update_code_analyzer(
                        codeanalyzer_jar_path
                    )
                    codeanalyzer_exec = shlex.split(
                        f"java -jar {codeanalyzer_jar_file}"
                    )
    ...
    def _init_codeanalyzer(self, analysis_level=1) -> JApplication:
        ...
        if self.analysis_json_path is None:
            ...
            analysis_json_path_file = Path(self.analysis_json_path).joinpath(
                "analysis.json"
            )
                ...
                try:
                    ...
                    if not analysis_json_path_file.exists():
                        raise CodeanalyzerExecutionException(
                            "Codeanalyzer did not generate the analysis file."
                        )
    ...
    def _codeanalyzer_single_file(self):
        ...
        try:
            ...
            console_out: CompletedProcess[str] = subprocess.run(
                codeanalyzer_cmd, capture_output=True, text=True, check=True
            )
    ...
    def _generate_call_graph(self, using_symbol_table) -> DiGraph:
        ...
        if using_symbol_table:
            NotImplementedError(
                "Call graph generation using symbol table is not implemented yet."
            )
        else:
            ...
            edge_list = [
                (
                    ...
                    (jge.target.method.signature, jge.target.klass),
                    {
                        "type": jge.type,
                        "weight": jge.weight,
                        "calling_lines": tsu.get_calling_lines(
                            jge.source.method.code, jge.target.method.signature
                        ),
                    },
                )
                for jge in sdg
    ...
    def get_call_graph_json(self) -> str:
        ...
        for edge in edges:
            ...
            callgraph_dict["source_method_body"] = self.call_graph.nodes[edge[0]][
                "method_detail"
            ].method.code
            callgraph_dict["source_class"] = edge[0][1]
            callgraph_dict["target_method_signature"] = edge[1][0]
            callgraph_dict["target_method_body"] = self.call_graph.nodes[edge[1]][
                "method_detail"
            ].method.code
    ...
    def get_all_callers(
        self, target_class_name: str, target_method_signature: str
    ) -> Dict:
        """
        ...
        caller_detail_dict["target_method"] = self.call_graph.nodes[
            (target_method_signature, target_class_name)
        ]["method_detail"]

        for source, target, data in in_edge_view:
            cm = {
                "caller_method": self.call_graph.nodes[source]["method_detail"],
    ...
    def get_all_callees(
        self, source_class_name: str, source_method_signature: str
    ) -> Dict:
        """
        ...
        out_edge_view = self.call_graph.out_edges(
            nbunch=(source_method_signature, source_class_name), data=True
        )

        callee_detail_dict["callee_details"] = []
        callee_detail_dict["source_method"] = self.call_graph.nodes[
            (source_method_signature, source_class_name)
        ]["method_detail"]
    ...
    def get_all_methods_in_class(self, qualified_class_name) -> Dict[str, JCallable]:
        ...
        methods = {
            k: v
            for (k, v) in ci.callable_declarations.items()
            if v.is_constructor is False
        }
        return methods
    ...
    def get_all_sub_classes(self, qualified_class_name) -> Dict[str, JType]:
        ...
        for cls in all_classes:
            if (
                qualified_class_name in all_classes[cls].implements_list
                or qualified_class_name in all_classes[cls].extends_list
            ):
    ...
    def get_all_fields(self, qualified_class_name) -> List[JField]:
        ...
        if ci is None:
            logging.warning(
                f"Class {qualified_class_name} not found in the application view."
            )
            return list()
    ...
    def get_all_nested_classes(self, qualified_class_name) -> List[JType]:
        ...
        return [
            self.get_class(c) for c in nested_classes
        ]  # Assuming qualified nested class names
    ...
    def get_class_call_graph(
        self, qualified_class_name: str, method_name: str | None = None
        ...
        if method_name is None:
            filter_criteria = {
                ...
                if node[1] == qualified_class_name
            }
        else:
            filter_criteria = {
                node
                for node in self.call_graph.nodes
                if tuple(node) == (method_name, qualified_class_name)
            }
    ...
    def get_all_entry_point_methods(self) -> Dict[str, Dict[str, JCallable]]:
        ...
        for k, v in class_dict.items():
            entry_point_methods = {
                method_name: callable_decl
                for (method_name, callable_decl) in v.callable_declarations.items()
                if callable_decl.is_entry_point is True
            }
    ...
    def get_all_entry_point_classes(self) -> Dict[str, JType]:
        ...
        for val in symtab.values():
            class_dict.update(
                (k, v)
                for k, v in val.type_declarations.items()
                if v.is_entry_point is True
            )
        return class_dict
